"use client"
import { Plus, GripVertical, Trash2, Play, FileText, Award, BookOpen, Calendar, MessageSquare } from "lucide-react"
...
import type { CurriculumModule, CurriculumTopic, CurriculumTemplate } from "@/types/curriculum"
...
export function CurriculumBuilder({ initialTemplate, modules, setModules }: CurriculumBuilderProps) {
  ...
  const updateModule = (moduleId: string, updates: Partial<CurriculumModule>) => {
  ...
  const addTopic = (moduleId: string) => {
    ...
    setModules(
      modules.map((module) => (module.id === moduleId ? { ...module, topics: [...module.topics, newTopic] } : module)),
    )
  }

  const updateTopic = (moduleId: string, topicId: string, updates: Partial<CurriculumTopic>) => {
    setModules(
      modules.map((module) =>
        module.id === moduleId
          ? {
  ...
  const addLearningObjective = (moduleId: string) => {
    const module = modules.find((m) => m.id === moduleId)
    if (module) {
      updateModule(moduleId, { learningObjectives: [...module.learningObjectives, ""] })
    }
  }
