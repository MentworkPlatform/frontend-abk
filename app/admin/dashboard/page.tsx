...
import { Users, Plus, School, Settings, Filter } from 'lucide-react'
...
export default function AdminDashboardPage() {
  ...
  const filteredMentors = mentors.filter((mentor) => {
    ...
    const matchesTier =
      tierFilter === 'all' ||
      mentor.tier.toLowerCase() === tierFilter.toLowerCase()
  ...
  const filteredPrograms = schoolPrograms.filter((program) => {
    return (
      programStatusFilter === 'all' || program.status === programStatusFilter
    )
  })
  ...
  return (
    <div className='container mx-auto py-6'>
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6'>
        <div>
          ...
          <p className='text-gray-500'>
            Manage mentors, programs, and platform settings
          </p>
        </div>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6'>
        ...
        <Card className='overflow-hidden border-none shadow-md'>
          <CardHeader className='pb-2 bg-gradient-to-r from-green-50 to-white'>
            <CardTitle className='text-sm font-medium text-gray-500 flex items-center'>
              <School className='mr-2 h-4 w-4 text-green-500' /> Active School
              Programs
            </CardTitle>
          </CardHeader>
          <CardContent className='pt-4'>
            <div className='text-2xl font-bold'>
              {
                schoolPrograms.filter(
                  (p) => p.status === 'in_progress' || p.status === 'open'
                ).length
              }
            </div>
            <div className='text-xs text-gray-500 mt-1'>
              {schoolPrograms.filter((p) => p.status === 'in_progress').length}{' '}
              in progress,{' '}
              {schoolPrograms.filter((p) => p.status === 'open').length} open
            </div>
          </CardContent>
        </Card>
        ...
        <Card className='overflow-hidden border-none shadow-md'>
          <CardHeader className='pb-2 bg-gradient-to-r from-purple-50 to-white'>
            ...
            <CardContent className='pt-4'>
              <div className='text-2xl font-bold'>
                {schoolPrograms.filter((p) => p.status === 'completed').length}
              </div>
              <div className='text-xs text-gray-500 mt-1'>
                {schoolPrograms.filter((p) => p.status === 'completed').length}{' '}
                completed
        ...
        <Card className='overflow-hidden border-none shadow-md'>
          <CardHeader className='pb-2 bg-gradient-to-r from-yellow-50 to-white'>
            ...
            <CardContent className='pt-4'>
              <div className='text-2xl font-bold'>
                {schoolPrograms.filter((p) => p.status === 'cancelled').length}
              </div>
              <div className='text-xs text-gray-500 mt-1'>
                {schoolPrograms.filter((p) => p.status === 'cancelled').length}{' '}
                cancelled
      ...
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6'>
        <div>
          <h2 className='text-lg font-bold tracking-tight'>Mentors</h2>
          <p className='text-gray-500'>Manage mentors and their profiles</p>
        </div>
        <div className='flex gap-2'>
          <Button variant='outline'>
      ...
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6'>
        <div>
          <h2 className='text-lg font-bold tracking-tight'>School Programs</h2>
          <p className='text-gray-500'>
            Manage school programs and their profiles
          </p>
        </div>
        <div className='flex gap-2'>
          <Button variant='outline'>
