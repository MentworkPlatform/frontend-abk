  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (data.success) {
          setStats((prev) => ({ ...prev, totalPrograms: data.programs.length }))
          setRecentPrograms(
            data.programs.slice(0, 3).map((program) => ({
              id: program.id,
              title: program.title,
              participants: program.maxParticipants || 25,
              mentors: program.mentorAssignments?.length || 4,
              progress: 75,
              status: program.status || 'active',
              startDate: program.startDate || '2024-01-15',
              revenue: program.price || 12500,
              hasLMS: program.curriculum && program.curriculum.length > 0,
            })),
          )
        }
      } catch (error) {
    ...
    <div className='flex-1 space-y-4 p-4 md:p-8 pt-6'>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Common tasks to help you manage your programs
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
